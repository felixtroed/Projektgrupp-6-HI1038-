...
#define CLIENT_ONE 0
#define CLIENT_TWO 1
#define CLIENT_THREE 2
#define CLIENT_FOUR 3
...
int main(int argc, char **argv) {
	...
	UDPpacket *pSend;			 /* Pointer to packet memory */
    Uint32 client1IP = 0;
    Uint32 client2IP = 0;
    Uint32 client3IP = 0;
    Uint32 client4IP = 0;
...
void sendData(UDPsocket sd, UDPpacket *pReceive, UDPpacket *pSend, udpData data, Uint32 clientIP, Uint32 clientPort) {
	...
	sprintf((char *)pSend->data, "%d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d\n", data->idx, data->x, data->y, data->frame, 
		data->nrOfClients, data->isHurt, 
		data->isDead, data->powerUpRow, data->powerUpCol, 
		data->bombDropped, data->bombPosX, data->bombPosY,
		data->explosionRange, data->leftBoxVal, data->leftBoxRow, 
		data->leftBoxCol, data->powerUpTaken, 
		data->rightBoxVal, data->rightBoxRow, data->rightBoxCol, 
		data->topBoxVal, data->topBoxRow, data->topBoxCol, 
		data->bottomBoxVal, data->bottomBoxRow, data->bottomBoxCol);
...
void initClient(UDPsocket sd, UDPpacket *pReceive, UDPpacket *pSend, Uint32 *clientIP, Uint32 *clientPort, udpData data) {
	...
	*clientPort = pReceive->address.port;
	...
	pSend->address.host = *clientIP; // sets client ip
	pSend->address.port = *clientPort; // sets clients port
	if (data->nrOfClients == 0) {
		printf("Client 1\n");
		sprintf((char *)pSend->data, "%d\n", CLIENT_ONE);  // Sets client to player 1
	}
	else if (data->nrOfClients == 1) {
		printf("Client 2\n");
		sprintf((char *)pSend->data, "%d\n", CLIENT_TWO);  // Sets client to player 2
	}
	else if (data->nrOfClients == 2) {
		printf("Client 3\n");
		sprintf((char *)pSend->data, "%d\n", CLIENT_THREE);  // Sets client to player 3
	}
	else if (data->nrOfClients == 3) {
		printf("Client 4\n");
		sprintf((char *)pSend->data, "%d\n", CLIENT_FOUR);  // Sets client to player 4
	}
